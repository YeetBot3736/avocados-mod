...
public class BedEntityRenderer implements BlockEntityRenderer<BedEntity> {
    private ModelPart bedHead;
    private ModelPart bedFoot;

    public BedEntityRenderer(BlockEntityRendererFactory.Context ctx) {
    ...
    @Override
    public void render(BedEntity bedBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
        ...
        if (world2 != null) {
            ...
            DoubleBlockProperties.PropertySource<BedEntity> propertySource = DoubleBlockProperties.toPropertySource(Avocados.BED_E, BedBlk::getBedPart, BedBlk::getOppositePartDirection, ChestBlock.FACING, blockState, world2, bedBlockEntity.getPos(), (world, pos) -> false);
            int k = ((Int2IntFunction)propertySource.apply(new LightmapCoordinatesRetriever())).get(i);
            this.renderPart(matrixStack, vertexConsumerProvider, blockState.get(BedBlk.PART) == BedPart.HEAD ? this.bedHead : this.bedFoot, blockState.get(BedBlk.FACING), spriteIdentifier, k, j, false);
        } else {
